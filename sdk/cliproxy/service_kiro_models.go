...
func (s *Service) fetchKiroModels(auth *coreauth.Auth) []*ModelInfo {
	...
	// ListAvailableModels is not available to API key credentials: the upstream
	// answers "your subscription does not support this application" even for a
	// key whose getUsageLimits call succeeds. Asking anyway would log a 403 on
	// every model listing, so the static catalog is used directly.
	if tokenData.AuthMethod == kiroauth.AuthMethodAPIKey {
		return registry.GetKiroModels()
	}
...
func extractKiroTokenData(auth *coreauth.Auth) *kiroauth.KiroTokenData {
	...
	return &kiroauth.KiroTokenData{
		...
		// The auth method travels with the token because an API key needs the
		// API_KEY token type header to be accepted by the models endpoint.
		AuthMethod: authString(auth, "auth_method"),
	}
}
