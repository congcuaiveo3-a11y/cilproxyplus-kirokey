...
// kiroAPIKeyRefreshInterval is how long an API key credential is left alone
// before the refresh manager looks at it again. An API key never rotates, so
// there is nothing to refresh; this only keeps the scheduler from re-examining
// the credential every few seconds.
const kiroAPIKeyRefreshInterval = 12 * time.Hour
...
func (e *KiroExecutor) executeWithRetry(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, accessToken, profileArn string, kiroPayload, body []byte, from, to sdktranslator.Format, reporter *usageReporter, currentOrigin, kiroModelID string, isAgentic, isChatOnly bool, tokenKey string) (cliproxyexecutor.Response, error) {
	...
	for endpointIdx := 0; endpointIdx < len(endpointConfigs); endpointIdx++ {
		...
		for attempt := 0; attempt <= maxRetries; attempt++ {
			...
			// Bearer credential for all auth types (Builder ID, IDC, social, API key).
...
// isKiroAPIKeyAuth reports whether the credential is a Kiro API key rather than
// an OAuth credential.
func isKiroAPIKeyAuth(auth *cliproxyauth.Auth) bool {
	...
	if auth.Metadata != nil {
		if method, ok := auth.Metadata["auth_method"].(string); ok && method == kiroauth.AuthMethodAPIKey {
			return true
		}
	...
	if auth.Attributes != nil {
		if auth.Attributes["auth_method"] == kiroauth.AuthMethodAPIKey {
			return true
		}
	}
	return false
...
// setKiroAuthorization applies the bearer credential for a Kiro request.
//
// An API key authenticates as itself and must be accompanied by the API_KEY
// token type; without that header the upstream treats the key as an OAuth access
// token and rejects every request. OAuth credentials keep the plain bearer form.
...
func getEffectiveProfileArnWithWarning(auth *cliproxyauth.Auth, profileArn string) string {
	...
	// An API key is bound to its profile upstream, so a missing ARN is normal
	// rather than a misconfiguration worth warning about.
	if isKiroAPIKeyAuth(auth) {
		return profileArn
	}
...
// kiroVersionFamilies lists the model families whose upstream IDs carry a
// dotted version, such as "claude-opus-4.8" and "gpt-5.6-terra". Model listing
// rewrites those dots to hyphens, so the request path has to put them back.
//
// Only the family prefix is listed, never a specific version: the major.minor
// pair is derived from whatever follows. Enumerating versions instead meant that
// every model Kiro added was advertised by the proxy and then rejected upstream
// with INVALID_MODEL_ID until this table caught up. Each entry ends at the
// character right before the version, which is why some keep a trailing hyphen
// and others (minimax-m, kimi-k) do not.
var kiroVersionFamilies = [...]string{
	"claude-opus-",
	"claude-sonnet-",
	"claude-haiku-",
	"minimax-m",
...
// normalizeKiroVersion restores the dotted version in a hyphenated Kiro model
// ID. Suffixes such as "-terra" and "-0309-reasoning" are preserved, while
// canonical dotted IDs, single-segment versions ("claude-sonnet-5") and IDs
// outside the known families pass through unchanged.
func normalizeKiroVersion(s string) string {
	for _, family := range kiroVersionFamilies {
		if !strings.HasPrefix(s, family) {
			continue
		}
		if dotted, ok := dotKiroVersionPair(s[len(family):]); ok {
...
// dotKiroVersionPair joins a leading "<major>-<minor>" pair with a dot and
// returns the rest of the identifier untouched. It reports false when the input
// does not open with two hyphen-separated digit runs, which is how a canonical
// dotted version or a non-version segment is left alone.
func dotKiroVersionPair(rest string) (string, bool) {
	major := kiroLeadingDigits(rest)
	if major == 0 || major >= len(rest) || rest[major] != '-' {
		return "", false
	}
	minorStart := major + 1
	minor := kiroLeadingDigits(rest[minorStart:])
	if minor == 0 {
		return "", false
	}
	end := minorStart + minor
	// A version pair ends the identifier or is followed by another segment; a
	// trailing digit run glued to something else is not a version.
	if end != len(rest) && rest[end] != '-' {
		return "", false
	}
	return rest[:major] + "." + rest[minorStart:], true
}

func kiroLeadingDigits(s string) int {
	i := 0
...
func (e *KiroExecutor) Refresh(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	// An API key is a static bearer credential: there is nothing to rotate, and
	// the refresh paths below all require a refresh token. Returning an error
	// here would mark the credential unavailable, so push the next check far out
	// instead and leave the credential usable.
	if isKiroAPIKeyAuth(auth) {
		log.Debugf("kiro executor: auth %s is an API key, skipping refresh", authID)
		updated := auth.Clone()
		updated.NextRefreshAfter = time.Now().Add(kiroAPIKeyRefreshInterval)
		return updated, nil
	}
...
func (e *KiroExecutor) fetchAndSaveProfileArn(ctx context.Context, auth *cliproxyauth.Auth, accessToken string) string {
	...
	// An API key's profile is bound to the key server-side and the lookup needs
	// OIDC client credentials the key does not have, so probing would only burn
	// a request per call and log a misleading failure.
	if isKiroAPIKeyAuth(auth) {
		log.Debugf("kiro executor: skipping profileArn fetch for api key auth")
		return ""
	}
