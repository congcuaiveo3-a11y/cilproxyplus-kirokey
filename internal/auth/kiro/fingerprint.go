...
// applyTokenType declares the API_KEY token type for Kiro API key credentials.
// The upstream otherwise reads the bearer value as an OAuth access token and
// rejects the request, so this header is required rather than advisory.
func applyTokenType(req *http.Request, authMethod string) {
