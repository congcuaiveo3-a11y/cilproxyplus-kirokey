...
// APIEndpoint returns the Q API endpoint to call for this credential.
//
// The profile ARN is the primary source because it names the region the profile
// actually lives in. An API key has no profile ARN, so its own Region field is
// the only thing that identifies its home region; without this fallback every
// API key request would be sent to us-east-1 and rejected.
func (t *KiroTokenData) APIEndpoint() string {
	if t == nil {
		return GetKiroAPIEndpoint("")
	}
	if region := ExtractRegionFromProfileArn(t.ProfileArn); region != "" {
		return GetKiroAPIEndpoint(region)
	}
	return GetKiroAPIEndpoint(strings.TrimSpace(t.Region))
}
